...
| `voice-local.js` | ES module: the **My voice profile** engine. A Worker-shaped `EventTarget` that answers the same messages as `voice-worker.js` by talking to a voice server on this machine (`http://127.0.0.1:7865`, see [`../voice-studio/`](../voice-studio/)): a fine-tuned 2B model of the speaker's own voice, where the in-browser clone is 100M parameters from ~10 s of the video. |
